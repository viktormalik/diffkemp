...
void ControlFlowSlicer::addAdditionalInsts(Function &Fun) {
...
void ControlFlowSlicer::addDebugInfo(Function &Fun) {
    for (auto &BB : Fun) {
        for (auto &Inst : BB) {
...
void ControlFlowSlicer::clearFunction(Function &Fun) {
...
void ControlFlowSlicer::setReturnBlock(Function &Fun) {
    UnifyFunctionExitNodes unifyExitPass;
    unifyExitPass.runOnFunction(Fun);
    RetBB = unifyExitPass.getReturnBlock();
}
...
std::vector<const BasicBlock *>
        ControlFlowSlicer::affectedBasicBlocks(BranchInst *Branch) {
...
void ControlFlowSlicer::addAllInstrs(
...
/// a dependent value.
bool ControlFlowSlicer::checkDependency(const Use *Op) {
    bool result = false;
...
bool ControlFlowSlicer::addToDependent(const Instruction *Instr) {
    return addToSet(Instr, DependentInstrs);
}

/// Add instruction to included instructions.
bool ControlFlowSlicer::addToIncluded(const Instruction *Inst) {
    if (!isDependent(Inst))
...
bool ControlFlowSlicer::addToSet(const Instruction *Inst,
...
/// \param Inst
/// \return
bool ControlFlowSlicer::addAllOpsToIncluded(const Instruction *Inst) {
    bool added = false;
...
bool ControlFlowSlicer::isAllocOrLoad(const Instruction *Inst) {
...
std::set<BasicBlock *>
        ControlFlowSlicer::includedSuccessors(BranchInst &Terminator,
...
/// basic block
bool ControlFlowSlicer::hasDependentPHI(BasicBlock *BB,
...
bool ControlFlowSlicer::canRemoveBlock(const BasicBlock *bb) {
...
bool ControlFlowSlicer::canRemoveFirstBlock(const BasicBlock *bb) {
...
std::set<const BasicBlock *>
        ControlFlowSlicer::reachableBlocks(const BasicBlock *Src,
                                           Function &Fun) {
...
std::set<const BasicBlock *>
        ControlFlowSlicer::reachableBlocksThroughSucc(Instruction *Terminator,
                                                      BasicBlock *Succ) {
...
void ControlFlowSlicer::intersectWith(
...
void ControlFlowSlicer::uniteWith(std::set<const BasicBlock *> &set,
                                  const std::set<const BasicBlock *> &other) {
    std::set<const BasicBlock *> tmpSet;
...
bool ControlFlowSlicer::isDependent(const Instruction *Instr) {
    return DependentInstrs.find(Instr) != DependentInstrs.end();
}

/// Check if an instruction must be included.
bool ControlFlowSlicer::isIncluded(const Instruction *Instr) {
...
bool ControlFlowSlicer::checkPhiDependency(const PHINode &Phi) {
    Value *Val = nullptr;
...
bool ControlFlowSlicer::addStoresToIncluded(const Instruction *Alloca,
...
void ControlFlowSlicer::deleteUnreachableBlocks(Function &Fun) {
...
/// ".old" suffix.
void ControlFlowSlicer::changeToVoid(Function &Fun) {
