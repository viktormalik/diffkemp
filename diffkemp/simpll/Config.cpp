...
Config::Config()
    ...
    if (!PatternConfigOpt.empty()) {
        // Parse --pattern-config option - path to pattern configuration.
        PatternConfigPath = PatternConfigOpt;
    }
...
Config::Config(std::string FirstFunName,
        ...
        : First(std::move(FirstModule)), Second(std::move(SecondModule)),
          ...
          CacheDir(CacheDir), PatternConfigPath(PatternConfigPath),
